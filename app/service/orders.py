...
class OrderTempService():
    ...
    def all(self):
        orders = OrderTemp.query.order_by(OrderTemp.id).all()
        return orders
    ...
    def find_order_by_product_id(self, product_id):
        order_temp = OrderTemp.query.filter_by(product_id=product_id).first()
        if order_temp:
